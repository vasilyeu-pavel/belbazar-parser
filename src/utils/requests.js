...
const getHeightAndSeason = (str) => str
  .split(' ')
  .map((i, j) => {
    // find '164'
    if (i.length === 3 && !Number.isNaN(+i)) {
      return ({
        height: i || 0,
    ...
    if (i.length > 3
          && i.includes('-')
          && i.split('-').every((h) => !Number.isNaN(+h))
    ) {
      return ({
        height: i || 0,
        season: str.split(' ')[j + 1] || '',
...
const filterByNeedFields = (list, host) => list.map(({
  ...
  return {
    ...
    pictures: gallery.split('"')
      ...
      .map((pictureUrl) => `https://belbazar24.by${pictureUrl}`),
    articul,
    indexid,
    sostav,
    cat_nazv,
